...
export async function getSearchOrderCompany({ order_name, order_status, track_id, company_id }) {
    const searchedOrder = await db.query(
        `SELECT * FROM orders
        WHERE order_status ILIKE($1) AND order_package ILIKE($2) OR parcel_track_id ILIKE($3) AND  delivery_company_id=$4`,
...
export async function getOrderAnanlyticsCompany(company_id) {
    const ordersCount = await db.query(`
      SELECT COUNT(*), (SELECT COUNT(*) FROM orders WHERE order_status = 'Pending' AND delivery_company_id = $1) AS pending_orders, (SELECT COUNT(*) FROM orders WHERE order_status = 'Delivered' AND delivery_company_id = $1) AS delivered_orders FROM orders o WHERE delivery_company_id = $1`,
        [company_id]);
    return {
        error: false,
...
export async function sendOrderActivities(field = {}) {
    const {
...
export async function getAllPendingOrderRepo() {
    const allOrder = await db.query(`
    SELECT p.track_number, p.parcel_remarks,p.parcel_photo, p.parcel_pickup_time,
